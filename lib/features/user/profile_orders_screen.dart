            ...
            Row(
              children: [
                ...
                Expanded(
                  flex: status == 'ready'
                      ? 1
                      : 2, // Se adapta si está el QR o no
                      ...
                      await Navigator.of(context).push(
                        MaterialPageRoute(
                          builder: (ctx) => OrderChatScreen(
                            orderId: doc.id,
                            isBusiness:
                                false, // FALSE porque somos el estudiante
                          ),
                        ),
                      );
    ...
    try {
      ...
      for (var item in items) {
        if (item is Map<String, dynamic> && item.containsKey('productId')) {
          ...
          if (productDoc.exists &&
              (productDoc.data()?['isAvailable'] ?? false)) {
      ...
      if (itemsAdded > 0) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text(
              'No se pudo reordenar. Productos no disponibles.',
            ),
  ...
  Future<void> _addFavoriteToCart(
    ...
    try {
      ...
      if (!productDoc.exists || !(productDoc.data()?['isAvailable'] ?? false)) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('Producto no disponible.'),
            backgroundColor: AppColors.warning,
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(12),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('"${favData['name']}" al carrito.'),
          backgroundColor: AppColors.success,
          behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(12),
